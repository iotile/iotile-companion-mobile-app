...
class PulseCounterController extends DeviceBaseController {
    public title: string;
    private ioInfo;
    ...
    constructor($stateParams, $injector, $scope, UIService) {
        super("pulseCounterCtrl", $stateParams.deviceId, $injector, $scope);

        this.UIService = UIService;
    }

    public async resetWidget(lid) {
        const label = this.ioInfo.label;
        const streamId = this.buildStreamID(lid);
        const internalStream = this.ioInfo.map[lid].resetConfigId;
...
angular
    .module("main")
    .controller("pulseCounterCtrl", PulseCounterController as any);
